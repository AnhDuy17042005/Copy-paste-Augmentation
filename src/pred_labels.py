...
from config import DATA_HATDIEU_DIR, MODEL_DIR, PRED_LABELS_DIR

# ========================== CONFIGS =================================

# PATH CONFIGS - folder ảnh gốc và folder output predict/label/object
SOURE_DIR      = DATA_HATDIEU_DIR
OUT_DIR        = PRED_LABELS_DIR

# MODEL CONFIGS - checkpoint YOLO/SAM và ngưỡng confidence
MODEL          = MODEL_DIR / "hatdieu_seg_ver2.pt"
SAM            = MODEL_DIR / "sam_vit_b_01ec64.pth"
CONFIDENT      = 0.5

# MASK SMOOTHING CONFIGS - xử lý mask sau predict để làm mượt polygon
...
# MASK POST-PROCESSING CONFIGS - bật/tắt các bước tinh chỉnh mask
BLUR           = True           
CLOSE          = True          
OPEN           = True           
FILL_HOLES     = True      
...
# FILTER CONFIGS - loại bỏ vùng mask quá nhỏ
MIN_AREA_RATIO = 0.001       # Tỷ lệ vật thể tối thiểu để lọc  

# ====================================================================

class PredLabelGenerator:
    def __init__(
        self,
    ...
    # Tạo file txt
    def label_obj(self, txt_path, instances, width, height):
    ...
    def morphological(self, binary_mask):
        mask = binary_mask
        if BLUR:
    ...
    def watershed(self, binary_mask):
    ...
    def remove_tiny(self, binary_mask, min_area_px):
    ...
    # drop hạt điều
    def extract_object(self, image_bgr, mask, out_path):
        h, w = mask.shape
    ...
    # Tạo smooth viền
    def smooth_binary_mask(self, binary_mask, w, h):

        # mượt + lấp lỗ trống + bỏ nhiễu nhỏ
        m = self.morphological(binary_mask)
        m = self.fill_holes(m)
        m = self.remove_tiny(m, int(MIN_AREA_RATIO*w*h))
        m = self.watershed(m)
    ...
    def run(self):
        ...
        cls_folders = [p for p in Path(self.source_dir).iterdir() if p.is_dir()]
        if not cls_folders:
            print(f"Không tìm thấy folder trong {self.source_dir}")
            return
        ...
        for cls_folder in cls_folders:
            ...
            predict_dir = Path(self.out_dir) / cls_name / "predict"
            labels_dir  = Path(self.out_dir) / cls_name / "labels"
            object_dir  = Path(self.out_dir) / cls_name / "object"
            ...
            for r in results:
                ...
                if r.masks is not None and len(r.masks) > 0:
                    ...
                    for i in range(num):
                        ...
                        polys = self.smooth_binary_mask(mask_sam, w, h)
                        ...
                        for k, poly in enumerate(polys, 1):
                            ...
                            self.extract_object(img_bgr, mm, drop)
            ...
            print(f"--- Xong {cls_name}, kết quả ở: {self.out_dir}/{cls_name}")


_DEFAULT_GENERATOR = PredLabelGenerator()


def label_obj(txt_path, instances, width, height):
    _DEFAULT_GENERATOR.label_obj(txt_path, instances, width, height)


def morphological(binary_mask):
    return _DEFAULT_GENERATOR.morphological(binary_mask)


def fill_holes(binary_mask):
    return _DEFAULT_GENERATOR.fill_holes(binary_mask)


def watershed(binary_mask):
    return _DEFAULT_GENERATOR.watershed(binary_mask)


def remove_tiny(binary_mask, min_area_px):
    return _DEFAULT_GENERATOR.remove_tiny(binary_mask, min_area_px)


def extract_object(image_bgr, mask, out_path):
    _DEFAULT_GENERATOR.extract_object(image_bgr, mask, out_path)


def smooth_binary_mask(binary_mask, w, h):
    return _DEFAULT_GENERATOR.smooth_binary_mask(binary_mask, w, h)


def main():
    _DEFAULT_GENERATOR.run()
